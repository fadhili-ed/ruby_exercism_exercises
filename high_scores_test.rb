...
class HighScoresTest < Minitest::Test
  ...
  def personal_top_highest_to_lowest
    ...
    scores = [20, 10, 30]
    expected = [30, 20, 10]
    assert_equal expected, HighScores.new(scores).personal_top_highest_to_lowest
  end
  ...
  def personal_top_when_there_is_a_tie
    ...
    scores = [40, 20, 40, 30]
    expected = [40, 40, 30]
    assert_equal expected, HighScores.new(scores).personal_top_when_there_is_a_tie
  end
  ...
  def personal_top_when_there_are_less_than_3
    ...
    scores = [30, 70]
    expected = [70, 30]
    assert_equal expected, HighScores.new(scores).personal_top_when_there_are_less_than_3
  end
  ...
  def personal_top_when_there_is_only_one
    ...
    scores = [40]
    expected = [40]
    assert_equal expected, HighScores.new(scores).personal_top_when_there_is_only_one
  end

  def latest_score_is_not_the_personal_best
    # skip
    scores = [100, 40, 10, 70]
    refute HighScores.new(scores).latest_score_is_not_the_personal_best
  end

  def latest_score_is_the_personal_best
    # skip
    scores = [70, 40, 10, 100]
    assert HighScores.new(scores).latest_score_is_the_personal_best
  end
end
